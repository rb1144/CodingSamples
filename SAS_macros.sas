...
%macro mfreq(dset, vars, label=Count, ttl=, order=, bal=, N=);
...
%macro goback(date, N, period, place, var, fmt=date9.);
%macro gplot(dset, by, vars, log=, labels=N, ylab=, range=Y, rangeLab=, join=Y,
%macro head(dset,r=50);
%macro holidays;
%macro import(file, dset, csv=Y, fld=\\NYCB\corp$\DBMarketing\Bogdan\data);
...
%macro rowCounts(tb, columns, order=);
...
%macro stack(ds, out);
%macro stats(dset, var, by=, print=Y, tall=Y, ttl=, fmt=, subset=);
%macro tail(dset,r=20, ttl=);
%macro timenowlog(message);
%macro toExcel(filename, sheet=Report);
%macro top(tb, N=200);
%macro transp(dset, by, id, vars, out, reverse=N);
%macro ts(dset, date, period=, v=count(*), vlab=freq, ttl=, plot=Y, fmt=);
